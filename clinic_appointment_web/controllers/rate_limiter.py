...
class ClinicRateLimit(http.Model):
    """Model to store rate limit attempts"""
    _name = 'clinic.rate_limit'
    _description = 'Rate Limit Tracking'
    _rec_name = 'client_id'

    client_id = http.fields.Char(
        string='Client ID',
        required=True,
        index=True,
    ...
    timestamp = http.fields.Datetime(
        string='Timestamp',
        required=True,
        index=True,
        default=http.fields.Datetime.now
    )

    ip_address = http.fields.Char(
        string='IP Address',
    ...
    def _gc_rate_limits(self):
        """Garbage collect old rate limit records"""
        # Keep records for 24 hours for security analysis
        cutoff = datetime.now() - timedelta(hours=24)
        self.search([('timestamp', '<', cutoff)]).unlink()

    _sql_constraints = [
        ('client_resource_time_unique',
         'UNIQUE(client_id, resource, timestamp)',
         'Duplicate rate limit entry'),
    ]